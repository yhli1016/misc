...
def vmod(x):
    ...
    :rtype: float
    """
    return math.sqrt(np.sum(x**2).item())


def correct_pbc(ref_image, chk_image, selected_atoms="all"):
    """
    ...
    :param List[int] selected_atoms: indices of atoms taken into consideration
    ...
    # Select all atoms if needed
    if selected_atoms == "all":
        selected_atoms = select_atoms(ref_image, exclude_atoms=None, offset=0)
    ...
    for i_a in selected_atoms:
        dist = [vmod(diff_pos[i_a] - r0) for r0 in neighbor_r0]
        chk_pos[i_a] += neighbor_r0[dist.index(min(dist))]
    chk_image.set_positions(chk_pos)


def align_image(ref_image, chk_image, selected_atoms="all"):
    """
    ...
    def _min_obj(x):
        return sum([vmod(diff_pos[i_a] - x)**2 for i_a in selected_atoms])
...
def center_image(image, center=0.5, selected_atoms="all"):
    """
    ...
    """
    if selected_atoms == "all":
        selected_atoms = select_atoms(image, exclude_atoms=None, offset=0)
...
def interpolate(initial_image, final_image, num_inter_images, interp="idpp",
                mic=True):
    """
    ...
    :param str interp: interpolation method
    :param bool mic: whether to enable minimal image correction for idpp
        interpolation
...
def main():
    ...
    # Correct final image
    fixed_atoms = select_atoms(image_ini, exclude_atoms=free_atoms,
                               offset=offset)
    if _correct_pbc:
        correct_pbc(image_ini, image_fin, fixed_atoms)
    if _align_image:
        align_image(image_ini, image_fin, fixed_atoms)

    # Center initial and final images
    if _center_image:
        center_image(image_ini, center_z, fixed_atoms)
        center_image(image_fin, center_z, fixed_atoms)

    # Interpolate
    mep = interpolate(image_ini, image_fin, num_inter_images, interp, mic)

    # Output
    if not debug:
        vasp_param = {"vasp5": True, "direct": True}
        os.system("rm -rf 0*")
        for i, image in enumerate(mep):
            dir_name = "%02d" % i
            os.mkdir(dir_name)
            write("%s/POSCAR" % dir_name, image, format="vasp", **vasp_param)
    write("mep.traj", mep)
