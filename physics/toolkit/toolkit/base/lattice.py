"""Functions for lattice and coordinate operations."""
...
def calc_geo_center(atom_coordinates):
    """
    Calculate the geometry center from atomic coordinates.

    :param atom_coordinates: N*3 array
    :return geo_center: 1*3 array
    """
    coord_sum = np.array([0.0, 0.0, 0.0])
    for row in atom_coordinates:
        coord_sum += row
    geo_center = coord_sum / len(atom_coordinates)
    return geo_center


def shift_geo_center(atom_coordinates, dimension, new_center):
    """
    Shift the geometry center to a new position.

    :param atom_coordinates: N*3 array
    :param dimension: dimension of the system, should be 0-3
    :param new_center: real, new position of the center
    :return new_coordinates: N*3 array
    """
    geo_center = calc_geo_center(atom_coordinates)
    if dimension == 0:
        shift_vector = [new_center-geo_center[i] for i in range(3)]
    elif dimension == 1:
        shift_vector = [new_center-geo_center[0], new_center-geo_center[1], 0.0]
    elif dimension == 2:
        shift_vector = [0.0, 0.0, new_center-geo_center[2]]
    else:
        shift_vector = [0.0, 0.0, 0.0]
    new_coorindates = atom_coordinates + np.array(shift_vector)
    return new_coorindates
