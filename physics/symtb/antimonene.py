...
from tbplas.base.lattice import cart2frac

from symtb import Model, HopDict
...
def make_antimonene(with_soc: bool = True,
    ...
    # SOC coupling matrix
    soc_mat = 0.5 * soc_lambda * sp.Matrix([
    ...
    # Deal with SOC
    if with_soc:
        hop_dict_soc = HopDict(cell.num_orb)
        for rel_unit_cell, hop in hop_dict.hoppings.items():
            new_hop = sp.zeros(12, 12)
            hop00 = hop[0:3, 0:3]
            hop01 = hop[0:3, 3:6]
            hop10 = hop[3:6, 0:3]
            hop11 = hop[3:6, 3:6]
            ...
            hop_dict_soc[rel_unit_cell] = new_hop
        hop_dict_soc[(0, 0, 0)] += TensorProduct(sp.eye(2), soc_mat)
        hop_dict = hop_dict_soc
...
def main():
    soc_lambda = sp.Symbol("soc_lambda", float=True)
    cell = make_antimonene(True, soc_lambda, 1.0)
    cell.print_cxx()
    cell.print_py()
