...
def cutoff_sc(super_cell: tb.SuperCell, 
              center: np.ndarray,
              radius: float = 3.0) -> None:
    """
...
def main():
    ...
    # Estimate dim for diamond-shaped prim_cell
    #
    # The radius of inscribed circle of a diamond is r = sqrt(3) / 4 * lat, with
    # lat being edge length. Since lat = a * sqrt(3), then r = 0.75 * a, with a
    # being C-C bond length.
    rmin_dia = math.ceil(radius / (0.75 * a)) + 1

    timer = tb.Timer()
    prim_cell = tb.make_graphene_diamond()
    dim = (rmin_dia, rmin_dia, 1)
    angle = 30 / 180 * math.pi
    center = np.array((2./3, 2./3, 0))
    timer.tic("prim_cell_diamond")
    cell = make_quasi_crystal_pc(prim_cell, dim, angle, center, radius, shift)
    timer.toc("prim_cell_diamond")
    ...
    sample = make_quasi_crystal_sample(pc_fixed, pc_twisted, dim, angle, center, radius, shift)
    timer.toc("sample_diamond")
    print(sample.num_orb, sample.num_hop)


    # Estimation of dim for rectangular prim_cell
    # This is easy since we need to consider the short edge only.
    pc_fixed = tb.make_graphene_rect()
    pc_twisted = tb.make_graphene_rect()
    rmin_rect = 2 * (math.ceil(radius / norm(pc_fixed.lat_vec[0])) + 1)
    dim = (rmin_rect, rmin_rect, 1)
