...
def extend_hop(model: Union[tb.PrimitiveCell, tb.SuperCell],
               max_distance: float = 0.75) -> None:
    """
    Extend the hopping terms in model up to cutoff distance.

    :param model: model to extend
    :param max_distance: cutoff distance in NM
    :return: None. Incoming model is modified
    """
    neighbors = tb.find_neighbors(model, a_max=1, b_max=1,
                                  max_distance=max_distance)
    for term in neighbors:
        i, j = term.pair
        model.add_hopping(term.rn, i, j, calc_hop(term.rij))


def make_layers(i: int = 5, shift: float = 0.3349) -> Tuple[tb.PrimitiveCell, tb.PrimitiveCell]:
    """
...
def make_tbg_pc(timer: tb.Timer, i: int = 5) -> None:
    timer.tic("make_tbg_pc")

    # Prepare the layers
    layer_fixed, layer_twisted = make_layers(i)
    ...
    # yield the hetero-structure.
        ...
        print(layer_fixed.num_hop, layer_twisted.num_hop, inter_hop.num_hop)
    ...
    if FAIR_PLAY:
        merged_cell._sync_array()
        #merged_cell.sync_array()
    timer.toc("make_tbg_pc")

    # Visualize Moire's pattern
    print("prim_cell", merged_cell.num_orb, merged_cell.num_hop)
