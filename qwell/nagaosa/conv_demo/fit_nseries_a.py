...
axes.set_ylabel(r"$\mathbf{a/a_B}$")
#axes.set_xlim([10, 140])
axes.set_ylim([0.680, 0.688])
axes.set_xticklabels([r"\textbf{%g}" % t for t in axes.get_xticks()])
axes.set_yticklabels([r"\textbf{%5.3f}" % t for t in axes.get_yticks()])
axes.minorticks_on()
axes.xaxis.set_minor_locator(ticker.MultipleLocator(5))
axes.yaxis.set_minor_locator(ticker.MultipleLocator(0.001))
